...
- Edge-case behavior of `findXConfig` methods in the `Config` class changed and methods are more consistent. The method `findCacheConfig` now behaves similarly to other `findXConfig` methods. PR 11780 ???
